...
@Component({
  selector: 'floor-view-delete-action',
  standalone: false,
  templateUrl: './floor-view-delete-action.component.html',
  styleUrl: './floor-view-delete-action.component.scss',
})
export class FloorViewDeleteActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete', {}, 'organization/floor').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.delete(this.selection()[0] as Floor);
        } else {
