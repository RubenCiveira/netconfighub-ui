...
@Component({
  selector: 'client-view-disable-action',
  standalone: false,
  templateUrl: './client-view-disable-action.component.html',
  styleUrl: './client-view-disable-action.component.scss',
})
export class ClientViewDisableActionComponent implements Action, OnDestroy {
  ...
  label = signal<string>('disable');
  icon = signal<string>('visibility_off');
  tooltip = signal<string>('disable el elemento seleccionado');
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.disable', {}, 'organization/client').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.disable(this.selection()[0] as Client);
        } else {
  ...
  private disable(item: Client) {
    this.repository.disable(item.uid).subscribe(() => {
      this.container()?.close();
      this.snakeBar.open(this.report, this.dismiss, { duration: 5000 });
    });
  }
  private allowed(allow: Acl | undefined) {
    return !!allow?.allows?.find((info) => info.name == 'disable')?.allowed;
  }
}
