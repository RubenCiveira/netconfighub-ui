...
@Component({
  selector: 'client-view-delete-action',
  standalone: false,
  templateUrl: './client-view-delete-action.component.html',
  styleUrl: './client-view-delete-action.component.scss',
})
export class ClientViewDeleteActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete', {}, 'organization/client').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.delete(this.selection()[0] as Client);
        } else {
