...
@Component({
  selector: 'client-view-disable-selection-action',
  standalone: false,
  templateUrl: './client-view-disable-selection-action.component.html',
  styleUrl: './client-view-disable-selection-action.component.scss',
})
export class ClientViewDisableSelectionActionComponent implements Action, OnDestroy {
  readonly name = 'disableSelection';
  ...
  icon = signal<string>('visibility_off');
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.disable-selection', {}, 'organization/client').subscribe((val) => {
      if (val.label) {
  ...
  visible(items: any[]) {
    const canApply = items.every((item) => item.enabled !== false) && true;
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.disableSelection(this.selection());
        } else {
  ...
  private allowed(allow: Acl | undefined) {
    return this.repository.canDeleteSelection() && !!allow?.allows?.find((info) => info.name == 'disable')?.allowed;
  }
}
