...
@Component({
  selector: 'site-view-delete-action',
  standalone: false,
  templateUrl: './site-view-delete-action.component.html',
  styleUrl: './site-view-delete-action.component.scss',
})
export class SiteViewDeleteActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete', {}, 'organization/site').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.delete(this.selection()[0] as Site);
        } else {
