...
@Component({
  selector: 'project-device-view-delete-action',
  standalone: false,
  templateUrl: './project-device-view-delete-action.component.html',
  styleUrl: './project-device-view-delete-action.component.scss',
})
export class ProjectDeviceViewDeleteActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete', {}, 'installations/project-device').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.delete(this.selection()[0] as ProjectDevice);
        } else {
  ...
  private delete(item: ProjectDevice) {
