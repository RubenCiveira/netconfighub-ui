...
import { DeviceInstanceProjection } from 'app/features/installations/device-instance/model/device-instance.model';
import { input, computed, effect, viewChild, Component } from '@angular/core';

@Component({
  standalone: false,
  selector: 'device-instance-view-standar-actions',
  templateUrl: './device-instance-view-standar-actions.component.html',
  styleUrl: './device-instance-view-standar-actions.component.scss',
})
export class DeviceInstanceViewStandarActionsComponent extends AbstactContainerWrapper {
  override actions = viewChild<ActionContainerComponent>('container');
  fixed = input<{ [key: string]: any }>();
  hidden = input<{ [key: string]: any }>();

  activated: Tick<DeviceInstanceProjection, string>;
  acl: Tick<{ generic: Acl; specific: Acl }, string | null>;

  mapper: ItemMapper<any>;

  constructor(public readonly repository: DeviceInstanceRepository) {
    super();
