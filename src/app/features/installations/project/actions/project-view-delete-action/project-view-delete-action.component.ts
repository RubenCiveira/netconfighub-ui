...
@Component({
  selector: 'project-view-delete-action',
  standalone: false,
  templateUrl: './project-view-delete-action.component.html',
  styleUrl: './project-view-delete-action.component.scss',
})
export class ProjectViewDeleteActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete', {}, 'installations/project').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.delete(this.selection()[0] as Project);
        } else {
