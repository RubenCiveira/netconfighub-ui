...
@Component({
  selector: 'device-check-view-delete-selection-action',
  standalone: false,
  templateUrl: './device-check-view-delete-selection-action.component.html',
  styleUrl: './device-check-view-delete-selection-action.component.scss',
})
export class DeviceCheckViewDeleteSelectionActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete-selection', {}, 'installations/device-check').subscribe((val) => {
      if (val.label) {
  ...
  private deleteSelection(items: DeviceCheck[]) {
