...
@Component({
  selector: 'device-check-view-delete-action',
  standalone: false,
  templateUrl: './device-check-view-delete-action.component.html',
  styleUrl: './device-check-view-delete-action.component.scss',
})
export class DeviceCheckViewDeleteActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete', {}, 'installations/device-check').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.delete(this.selection()[0] as DeviceCheck);
        } else {
  ...
  private delete(item: DeviceCheck) {
