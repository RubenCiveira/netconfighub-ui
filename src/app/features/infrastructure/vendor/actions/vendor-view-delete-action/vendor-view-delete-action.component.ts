...
@Component({
  selector: 'vendor-view-delete-action',
  standalone: false,
  templateUrl: './vendor-view-delete-action.component.html',
  styleUrl: './vendor-view-delete-action.component.scss',
})
export class VendorViewDeleteActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete', {}, 'infrastructure/vendor').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.delete(this.selection()[0] as Vendor);
        } else {
