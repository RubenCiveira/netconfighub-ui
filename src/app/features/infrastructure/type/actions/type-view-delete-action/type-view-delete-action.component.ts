...
@Component({
  selector: 'type-view-delete-action',
  standalone: false,
  templateUrl: './type-view-delete-action.component.html',
  styleUrl: './type-view-delete-action.component.scss',
})
export class TypeViewDeleteActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete', {}, 'infrastructure/type').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.delete(this.selection()[0] as Type);
        } else {
