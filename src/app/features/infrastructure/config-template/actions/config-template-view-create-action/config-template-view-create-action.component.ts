...
@Component({
  selector: 'config-template-view-create-action',
  standalone: false,
  templateUrl: './config-template-view-create-action.component.html',
  styleUrl: './config-template-view-create-action.component.scss',
})
export class ConfigTemplateViewCreateActionComponent implements Action {
  ...
  title = signal<string>('Creando nuevo config template');
  ...
  form = new FormControl<ConfigTemplate | undefined>(undefined);
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.create', {}, 'infrastructure/config-template').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    this.form.patchValue({} as ConfigTemplate);
  }
  save() {
