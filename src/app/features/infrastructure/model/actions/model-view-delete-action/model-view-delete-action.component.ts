...
@Component({
  selector: 'model-view-delete-action',
  standalone: false,
  templateUrl: './model-view-delete-action.component.html',
  styleUrl: './model-view-delete-action.component.scss',
})
export class ModelViewDeleteActionComponent implements Action, OnDestroy {
  ...
  constructor(
    ...
    transloco.selectTranslateObject('action.delete', {}, 'infrastructure/model').subscribe((val) => {
      if (val.label) {
  ...
  run(): void {
    ...
    if (template) {
      ...
      this.dialogSubs = this.dialgoRef.afterClosed().subscribe((val) => {
        if (val) {
          this.delete(this.selection()[0] as Model);
        } else {
