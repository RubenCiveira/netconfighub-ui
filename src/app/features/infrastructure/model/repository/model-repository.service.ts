/* @autogenerated */
import { DisplayProvider } from '@common/data/display-provider';
import { ModelFilter, ModelProjection, Model } from '@features/infrastructure/model/model/model.model';
import { selectorProvider, SelectorProvider } from '@common/data/selector-provider';
import { Acl } from '@common/data/acl/acl.model';
import { RepositoryMetadata, RepositoryService } from '@common/data/repository/repository.service';
import { VendorRepository } from '@features/infrastructure/vendor/repository/vendor-repository.service';
import { Slide } from '@common/data/repository/provider/repository-provider.interface';
import { ApiProvider } from '@common/data/repository/provider/impl/api.provider';
import { ApiService } from '@common/data/api/api.service';
import { Observable, of } from 'rxjs';
import { TypeRepository } from '@features/infrastructure/type/repository/type-repository.service';
import { Shift, Tick } from '@common/data/repository/repository.model';
import { Injectable, Signal } from '@angular/core';

@Injectable({ providedIn: 'root' })
export class ModelRepository {
  private readonly repo: RepositoryService<Model, ModelFilter>;
  public static metadata(api: ApiService): RepositoryMetadata<Model, ModelFilter> {
    return {
      key: 'uid',
      name: 'model',
      provider: new ApiProvider(
        'uid',
        api,
        'http://net-civeira-netconfighub/infrastructure/models',
        'http://net-civeira-netconfighub/me/acl/infrastructure/models',
      ),
      reference: (name: string): RepositoryService<any, any> | null => {
        if ('vendor' == name) {
          return new RepositoryService(VendorRepository.metadata(api));
        }
        if ('type' == name) {
          return new RepositoryService(TypeRepository.metadata(api));
        }
        return null;
      },
    };
  }
  constructor(api: ApiService) {
    this.repo = new RepositoryService(ModelRepository.metadata(api));
  }
  acl(): Tick<{ generic: Acl; specific: Acl }, string | null> {
    return this.repo.acl();
  }
  checkAcl(key?: string): Observable<Acl> {
    return this.repo.checkAcl(key || null);
  }
  generateComb(): string {
    return this.repo.generateComb();
  }

  add(item: Model): Observable<Model> {
    item['uid'] = this.generateComb();
    return this.repo.add(item);
  }
  update(item: Model): Observable<Model> {
    return this.repo.update(item);
  }
  deleteByKey(item: string): Observable<void> {
    return this.repo.deleteByKey(item);
  }
  canDeleteSelection(): boolean {
    return this.repo.canDeleteSelection();
  }
  deleteSelection(filter: ModelFilter): Observable<any> {
    return this.repo.deleteSelection(this.publicQueryFilter(filter));
  }
  retrieve(code: string): Observable<Model | undefined> {
    return this.repo.retrieve(code);
  }
  projection(includes: string[]): Tick<ModelProjection, string> {
    return this.repo.projection(includes);
  }
  list(filter: ModelFilter): Observable<Slide<Model>> {
    return this.repo.list(this.publicQueryFilter(filter));
  }
  values(filter: ModelFilter): Observable<Model[]> {
    return this.repo.values(this.publicQueryFilter(filter));
  }
  projections(includes: string[]): Shift<ModelProjection[], ModelFilter> {
    return this.repo.projections(includes);
  }
  displayProvider(): DisplayProvider<Model> {
    return {
      display: (item: Model) => {
        return item.name!;
      },
    };
  }
  selectorProvider(filter?: Signal<ModelFilter>): SelectorProvider<Model, { $ref: string }> {
    return selectorProvider({
      search: (text: string) => {
        const filterValue = filter ? filter() : { limit: 10 };
        filterValue.search = text;
        return this.values(filterValue);
      },
      display: (item: Model) => {
        return item.name!;
      },
      value: (item: Model) => {
        return { $ref: item.uid! };
      },
      select: (ref: { $ref: string }) => {
        return ref['$ref'] ? this.retrieve(ref['$ref']) : of(undefined);
      },
      equals: (one: { $ref: string }, other: { $ref: string }) => {
        return one.$ref === other.$ref;
      },
    });
  }
  private publicQueryFilter(filter: ModelFilter): ModelFilter {
    const morphed = {} as any;
    if (filter.search) {
      morphed['search'] = filter.search;
    }
    if (filter.uids) {
      morphed['uids'] = filter.uids.join(',');
    }
    if (filter.limit) {
      morphed['limit'] = filter.limit;
    }
    if (filter.order) {
      morphed['order'] = filter.order;
    }
    if (filter.name) {
      morphed['name'] = filter.name;
    }
    if (filter.vendor) {
      morphed['vendor'] = filter.vendor;
    }
    if (filter.vendors) {
      morphed['vendors'] = filter.vendors;
    }
    if (filter.type) {
      morphed['type'] = filter.type;
    }
    if (filter.types) {
      morphed['types'] = filter.types;
    }
    return morphed;
  }
}
