    ...
    <ng-template #contextualTemplate let-row="row">
      ...
      @if (inlineMenu(row).length > 0) {
        <button
          ...
          (click)="showMenu(row); $event.stopPropagation()"
        >
    ...
    <ng-template #activeTemplate>
      <div class="toolbar">
          ...
          @for (step of breadcrumb; track $index) {
            <div class="bread-step">
              ...
              <div class="bread-step-subtitle">
                {{
                  step.display() ? step.display()?.display(step.selectionModel.selected()[0]) : step.current()?.title()
                }}
                <button mat-icon-button aria-label="{{ moreOptionsLabel() }}" [matMenuTriggerFor]="breadDialogMenu">
        ...
        @if (leaf.dialogMenu().length > 0) {
          <button mat-icon-button aria-label="{{ moreOptionsLabel() }}" [matMenuTriggerFor]="theDialogMenu">
            <mat-icon>more_vert</mat-icon>
          </button>
...
export class ActionContainerComponent implements OnDestroy {
  ...
  moreOptionsLabel = signal('More actions');
